...
let sensors=[],sortKey='id',sortDir=1;
...
function keyVal(s,key){
  switch(key){
    ...
    case'timestamp':return s.latest?.timestamp??0;
    default:return'';
  }
}
...
function buildRows(s){
  ...
  /* summary row */
  const row=document.createElement('tr');row.className='sensor-row';
  row.innerHTML=`
    <td>${id}</td>
    <td>${name??'—'}</td>
  ...
  /* detail row */
  const detail=document.createElement('tr');detail.className='sensor-detail';
